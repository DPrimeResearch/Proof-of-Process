...
boolean checkTooTired(int servo) {
  servo--;
...
String padDegree(String degree) {
  String degreeString = degree;
  //need to make sure degree is 3 characters, pad with zeros
  if (degree.length() == 1) {
  degreeString="00"+degree;
...
void sendTest(int servoNum, int blueVal, int greenVal) {
...
void sendOSC(int servoNum, int blueVal, int greenVal) {
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/fromsorrykeyboard")) {
    if(theOscMessage.checkTypetag("ii")) {
      ...
      servo = int(map(servo, 21, 108, 1, 4));
  ...
  if (theOscMessage.checkAddrPattern("/fromprotocol")) {
    if(theOscMessage.checkTypetag("i")) {
      ...
      servoProtocol = int(map(servoProtocol, 30, 200, 1, 4));
    }
  }
  ...
  if (servoProtocol == 999 && degBiopoesis1 != 999 && degBiopoesis2 != 999) {
      ...
      degBiopoesis1 = int(map(degBiopoesis1, 0, 255, 1, 4));
...
void serialEvent (Serial myPort) { //wait for serial data and send out using OSC
      ...
      if (!myString.equals("0000") && !myString.equals("000000")) {
        ...
        if (myInts[0] != 0) {
          ...
          if (myInts[1] < 10) {
            ...
            int colorVal = int(map(myInts[1], 0, 1023, 0, 255));
            float blueness = 0.70;
            float greenness = 0.30;
          ...
          sendOSC (outmessage, int(blueness*colorVal), int(greenness*colorVal));
